...
class ExtractPagesViewModel @Inject constructor(
    ...
    fun extractPages() {
        ...
        viewModelScope.launch {
            ...
            val tempFile = File(pdfEngine.cacheDir, "extracted_${System.currentTimeMillis()}.pdf")
            
            val result = pdfEngine.extractPages(sourceUri, pages, tempFile)
            result.fold(
                onSuccess = { file ->
                    try {
                        ...
                        _error.value = e.message ?: "Failed to save extracted PDF"
                    }
                },
                onFailure = { error ->
                    _error.value = error.message ?: "Failed to extract pages"
                }
            )
