...
class PdfEngine @Inject constructor(
     ...
     * All InputStreams are kept open until mergeDocuments() completes,
     * then closed in a finally block to prevent resource leaks.
     */
        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                for ((index, uri) in uris.withIndex()) {
                    ...
                    // Faux progress for loading phase (0-50%)
    ...
    /**
     * Splits a PDF into individual single-page documents.
     * Each split PDDocument is closed via .use {} immediately after saving.
     */
    ...
    /**
     * Compresses a PDF by downsizing all embedded images to 50% dimensions
     * and re-encoding at 0.5 JPEG quality.
     *
     * MEMORY SAFETY: Every Bitmap is .recycle()'d immediately after use
     * to prevent OOM on heavy image-based PDFs.
     */
                    ...
                    for (page in document.pages) {
                        ...
                        for (name in resources.xObjectNames) {
                            ...
                            if (xObject is PDImageXObject) {
                                ...
                                val scaledWidth =
                                    (originalBitmap.width * 0.5f).toInt().coerceAtLeast(1)
                                val scaledHeight =
                                    (originalBitmap.height * 0.5f).toInt().coerceAtLeast(1)
                                val scaledBitmap = Bitmap.createScaledBitmap(
                                    originalBitmap, scaledWidth, scaledHeight, true
                                )
                                originalBitmap.recycle() // CRITICAL: recycle immediately — prevents OOM

                                val compressedImage =
                                    JPEGFactory.createFromImage(document, scaledBitmap, 0.5f)
                                scaledBitmap.recycle() // CRITICAL: recycle scaled bitmap too
    ...
    /**
     * Copies a single cache file to a user-chosen SAF destination URI.
     */
    suspend fun copyToUri(sourceFile: File, destinationUri: Uri): Result<Unit> = runCatching {
        withContext(Dispatchers.IO) {
            ...
            } ?: throw IllegalStateException("Unable to save file")
        }
    }

    /**
     * Copies multiple cache files to a user-chosen SAF directory.
     * Returns a list of (fileName, documentUri) pairs for database synchronization.
     */
    suspend fun copyToFolder(sourceFiles: List<File>, folderUri: Uri): Result<List<Pair<String, Uri>>> = runCatching {
        withContext(Dispatchers.IO) {
            val folder = DocumentFile.fromTreeUri(context, folderUri)
                ?: throw IllegalStateException("Unable to access folder")
    ...
    suspend fun imagesToPdf(uris: List<Uri>, outputCacheFile: File): Result<File> = runCatching {
        withContext(Dispatchers.IO) {
            PDDocument().use { document ->
                for (uri in uris) {
                    // Pass 1: Decode bounds to calculate safe downsampling
    ...
    suspend fun extractTextFromImage(uri: Uri): Result<String> = runCatching {
        withContext(Dispatchers.IO) {
        ...
        withContext(Dispatchers.IO) {
            context.contentResolver.openInputStream(uri)?.use { inputStream ->
                PDDocument.load(inputStream).use { document ->
                    ...
                    // Filter out invalid pages and sort in descending order (0-based indexing)
            ...
            context.contentResolver.openInputStream(uri)?.use { inputStream ->
                PDDocument.load(inputStream).use { document ->
                    ...
                    for (edit in edits) {
                        when (edit) {
                            is PdfEdit.Draw -> {
                                val annotationDict = com.tom_roush.pdfbox.cos.COSDictionary()
                                annotationDict.setName(com.tom_roush.pdfbox.cos.COSName.TYPE, "Annot")
                                annotationDict.setName(com.tom_roush.pdfbox.cos.COSName.SUBTYPE, "Ink")
                                
                                val pathMeasure = android.graphics.PathMeasure(edit.path, false)
                                ...
                                val pathsArray = com.tom_roush.pdfbox.cos.COSArray()
                                val pointArray = com.tom_roush.pdfbox.cos.COSArray()
                                coords.forEach { pointArray.add(com.tom_roush.pdfbox.cos.COSFloat(it)) }
                                pathsArray.add(pointArray)
                                annotationDict.setItem(com.tom_roush.pdfbox.cos.COSName.getPDFName("InkList"), pathsArray)
                                
                                val colorArray = com.tom_roush.pdfbox.cos.COSArray()
                                colorArray.add(com.tom_roush.pdfbox.cos.COSFloat(android.graphics.Color.red(edit.color) / 255f))
                                colorArray.add(com.tom_roush.pdfbox.cos.COSFloat(android.graphics.Color.green(edit.color) / 255f))
                                colorArray.add(com.tom_roush.pdfbox.cos.COSFloat(android.graphics.Color.blue(edit.color) / 255f))
                                annotationDict.setItem(com.tom_roush.pdfbox.cos.COSName.C, colorArray)
                                
                                val bsDict = com.tom_roush.pdfbox.cos.COSDictionary()
                                bsDict.setFloat(com.tom_roush.pdfbox.cos.COSName.W, edit.strokeWidth)
                                annotationDict.setItem(com.tom_roush.pdfbox.cos.COSName.BS, bsDict)
                                ...
                                annotationDict.setItem(com.tom_roush.pdfbox.cos.COSName.RECT, rect.cosArray)
                                
                                val annotation = com.tom_roush.pdfbox.pdmodel.interactive.annotation.PDAnnotationUnknown(annotationDict)
                                page.annotations.add(annotation)
                            }
                            is PdfEdit.Highlight -> {
                                val annotation = com.tom_roush.pdfbox.pdmodel.interactive.annotation.PDAnnotationTextMarkup(
                                ...
                                val color = com.tom_roush.pdfbox.pdmodel.graphics.color.PDColor(
                                    floatArrayOf(
                                        android.graphics.Color.red(edit.color) / 255f,
                                        android.graphics.Color.green(edit.color) / 255f,
                                        android.graphics.Color.blue(edit.color) / 255f
                                    ),
                            ...
                            is PdfEdit.Text -> {
                                com.tom_roush.pdfbox.pdmodel.PDPageContentStream(
                                    ...
                                    stream.beginText()
                                    stream.setFont(com.tom_roush.pdfbox.pdmodel.font.PDType1Font.HELVETICA, edit.size)
                                    stream.setNonStrokingColor(
                                        android.graphics.Color.red(edit.color),
                                        android.graphics.Color.green(edit.color),
                                        android.graphics.Color.blue(edit.color)
                                    )
    ...
    suspend fun htmlToPdf(htmlContent: String, outputCacheFile: File): Result<File> = runCatching {
        val adapter = withContext(Dispatchers.Main) {
            suspendCancellableCoroutine<android.print.PrintDocumentAdapter> { continuation ->
                val webView = android.webkit.WebView(context)
                webView.webViewClient = object : android.webkit.WebViewClient() {
                    override fun onPageFinished(view: android.webkit.WebView?, url: String?) {
                        super.onPageFinished(view, url)
                        continuation.resume(webView.createPrintDocumentAdapter("document")) {}
                    }
                }
                // Determine if it's a URL or raw HTML
        ...
        withContext(Dispatchers.IO) {
            suspendCancellableCoroutine<File> { continuation ->
                android.print.PdfPrinter.print(
                    adapter,
                    outputCacheFile,
                    onSuccess = { continuation.resume(outputCacheFile) {} },
    ...
    suspend fun extractTextToString(uri: Uri): Result<String> = runCatching {
        withContext(Dispatchers.IO) {
            android.util.Log.d("PdfEngine_Debug", "Extracting text from URI: $uri")
            try {
                context.contentResolver.openInputStream(uri)?.use { inputStream ->
                    PDDocument.load(inputStream).use { document ->
                        val stripper = PDFTextStripper()
                        val extracted = stripper.getText(document)
                        android.util.Log.d("PdfEngine_Debug", "Extracted text length: ${extracted.length} chars")
                        extracted
                    }
                } ?: throw IllegalStateException("Unable to open PDF")
            } catch (e: Exception) {
                com.google.firebase.crashlytics.FirebaseCrashlytics.getInstance().recordException(e)
                android.util.Log.e("PdfEngine_Debug", "Failed to extract text from PDF", e)
                throw e
            }
        }
    }
    ...
    suspend fun getPageCount(uri: Uri): Result<Int> = runCatching {
        withContext(Dispatchers.IO) {
            ...
            } ?: throw IllegalStateException("Unable to open PDF for page count")
        }
    }
